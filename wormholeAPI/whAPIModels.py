...
from wormholeAPI.whDataModels import whSequenceData
from wormholeAPI.whDataModels import whShotData
from wormholeAPI.whDataModels import whAssetData
from wormholeAPI.whDataModels import whShotHistData
from wormholeAPI.whDataModels import whAssetHistData
from wormholeAPI.whDataModels import whMemberData
from wormholeAPI.whDataModels import whProjectPathData
from wormholeAPI.whDataModels import whEnvData
from wormholeAPI.whDataModels import whTaskListData
from wormholeAPI.whDataModels import whPAssetData
from wormholeAPI.whDataModels import whPShotData
from wormholeAPI.whDataModels import whAssetRData

from wormholeAPI.urlModels import PROJECT_LIST_QUERY
from wormholeAPI.urlModels import SEQ_LIST_QUERY
from wormholeAPI.urlModels import SHOT_NAME_LIST_QUERY
from wormholeAPI.urlModels import ASSET_ID_LIST_QUERY
from wormholeAPI.urlModels import SHOT_LIST_QUERY
from wormholeAPI.urlModels import ASSET_LIST_QUERY
from wormholeAPI.urlModels import PROJECT_USER_LIST_QUERY
from wormholeAPI.urlModels import USER_INFO_QUERY
from wormholeAPI.urlModels import GET_INFO_QUERY
from wormholeAPI.urlModels import SHOT_VERSION_LIST_QUERY
from wormholeAPI.urlModels import CallUrl
from wormholeAPI.urlModels import PROJECT_FILE_PATH_LIST_QUERY
from wormholeAPI.urlModels import GET_TASK_TYPE_LIST_QUERY
from wormholeAPI.urlModels import PUBLISH_ASSET_QUERY
from wormholeAPI.urlModels import PUBLISH_SHOT_QUERY
from wormholeAPI.urlModels import RECORD_IMPORT_ASSET_QUERY
from wormholeAPI.urlModels import FileUpload
from resourceIDs import WH_PRJ
...
class whUser(whMemberData, whBase):
    ...
    def __getUserInfo(self):
        ...
        hlquery.setMustQuery(self.corpPrefix, self.userId)
...
class Wh(object):
    ...
    def getProject(self, corpPrefix, projectid):
        '''get whproject object with companyid and project id'''
        whcompany = whCompany(corpPrefix)
        whproject = whcompany.getProject(projectid)
        self._query = whcompany.query
        self._result = whcompany.result
        return whproject

    def getAsset(self, corpPrefix, projectId, assetid):
        aquery = ASSET_LIST_QUERY()
        aquery.setMustQuery(corpPrefix, projectId, assetid)
        self._query = aquery.getQuery()
        self.quety_call = CallUrl()
        self.quety_call.setURL(self._query)
        ...
        if self.quety_call.getIsSuccess():
            ...
            if self._result['AssetList']:
                asset_ = self._result['AssetList'][0]
                whasset = whAsset( corpPrefix, projectId, asset_)
                return whasset
        return None

    def getSequence(self, corpPrefix, projectId, sequenceId):
        plquery = SEQ_LIST_QUERY()
        plquery.setMustQuery(corpPrefix, projectId)
        self._query = plquery.getQuery()
        self.quety_call  = CallUrl()
        self.quety_call.setURL(self._query)
        ...
        if self.quety_call.getIsSuccess():
            ...
            if self._result['sequenceList']:
                for seq_ in self._result['sequenceList']:
                    if seq_['sequenceId'] == sequenceId:
                        whsequence = whSequence(corpPrefix, projectId, seq_)
                        return whsequence
        return None

    def getShot(self,  corpPrefix, projectId, shotid):
        slquery = SHOT_VERSION_LIST_QUERY()
        slquery.setMustQuery(corpPrefix, projectId, shotid)
        self._query = slquery.getQuery()
        ...
        if self.quety_call.getIsSuccess():
            ...
            whshot = whShot(corpPrefix, projectId, self._result)
            return whshot
        return None

    def getMember(self, corpPrefix, projectId, userid):
        plquery = PROJECT_USER_LIST_QUERY()
        plquery.setMustQuery(corpPrefix, projectId)
        self._query = plquery.getQuery()
        self.quety_call  = CallUrl()
            ...
            if self._result['UserList']:
                for user_ in self._result['UserList']:
                    if user_.userId == userid:
                        whuser = whUser(corpPrefix, projectId, user_)
                        return whuser
        return None
